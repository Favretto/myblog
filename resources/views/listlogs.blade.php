		...
		<!--
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="flex justify-center pt-8 sm:justify-start sm:pt-0">
					...
					<div style="display: flex; align-items: center; gap: 20px;">
						...
						<div style="display: flex; flex-direction: column; justify-content: center;">
							<h2 style="font-size: 48px; font-weight: bold; color: yellow; margin: 0;">Storico Attività</h2>
						</div>
					</div>
                </div>
				...
				@foreach($logcollection as $log)
					<div style="margin: 10px 0; padding: 10px; border: 1px solid #000; background-color: #00fff0;">
						<p><strong>Log id:</strong> {{ $log->id }}</p>
						<p><strong>Blog id:</strong> {{ $log->blog_id }}</p>
						<p><strong>User id:</strong> {{ $log->user_id }}</p>
						<p><strong>Azione:</strong> {{ $log->azione }}</p>
						<p><strong>Dati RAW:</strong> {{ json_encode($log->dati, JSON_PRETTY_PRINT) }}</p>
						<p><strong>Data Creazione:</strong> {{ $log->created_at }}</p>
					</div>
				@endforeach
